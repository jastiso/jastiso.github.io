...
modeling methods relied on assuming a specific form of the data rather than learning it, and didn't allow with data with (DEFINITION OF COMPLEXITY), making them less useful in
 many modern data settings. DML allows these two to coexist, which is the source of much of the hype in economics. 
...
1.)
2.)
3.) You are recording all variables that influence Y and T in X. I think this is the most fraught assumption in medical contexts (REF).

Caveats:
1.) categorical treatment
2.) biased data classes
...
These equations are a useful example because they give us a specific functional form for how T affects Y (T times theta). Since this relationship is linear,
...
Let's imaging you haven't read this blog post and you given some X,T, and Y data. You're told that there is definitely a linear relationship between T and
Y. You're then asked to estimate what that relationship is exactly. They tell you they know the answer, but want to see if you can find it on your own. 
If you're like me, your first instinct might be to estimate theta exactly. Specifically, you could use your machine learning model of choice 
(for me, random forest) to estimate Y from X, then regress out the effect of T. 

Mathematically, you can do this by moving some terms around to isolate theta
...
Now we can see how well we did! If our estimate is good, we would expect the normalized difference between our estimate of theta, and the real theta to
 be centered on 0.
 
HISTOGRAM IMAGE

This histogram shows that is not the case. Our estimate is centered on a slightly positive value. What went wrong?
...
Illustrating this explicitly is where our partially linear data generating process comes in handy. We can write out an equation for the error in our estimate.
 As a reminder, the goal here would be for this term to converge to 0 as we get more data.
...
We have to remove theeffect of X on T to circumvent this bias. We can do this in three steps:
